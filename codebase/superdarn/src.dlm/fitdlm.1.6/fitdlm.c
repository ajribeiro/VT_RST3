...
static IDL_VPTR IDLFitSeek( int argc, IDL_VPTR *argv, char *argk )
{
  ...
  if ( vatme )
    IDL_StoreScalar( vatme, IDL_TYP_DOUBLE, ( IDL_ALLTYPES * ) &atme );
...
static IDL_VPTR IDLFitOpen( int argc, IDL_VPTR *argv, char *argk )
{
  ...
  if ( iread != 0 )
    access = access | IDL_OPEN_R;

  if ( iwrite != 0 )
    access = access | IDL_OPEN_W;

  if ( iupdate != 0 )
    access = access | IDL_OPEN_APND;
