...
static IDL_MSG_DEF msg_arr[] =
  {
    ...
    {  "RPOS_BADRNGARR","%NBeam and range arrays must be of equa length %s"},
    {  "RPOS_BADFRANGARR","%NBeam and frang arrays must be of equa length %s"},
    {  "RPOS_BADRSEPARR","%NBeam and rsep arrays must be of equa length %s"},
    {  "RPOS_BADRXRISEARR",
       "%NBeam and rxrise arrays must be of equa length %s"},
    {  "RPOS_BADHEIGHTARR",
        "%NBeam and height arrays must be of equa length %s"},
...
static IDL_VPTR IDLRadarEpochGetSite(int argc,IDL_VPTR *argv) {
  ...
  if ((iradar->st_time !=-1) && (tval<iradar->st_time)) return NULL;
  if ((iradar->ed_time !=-1) && (tval>iradar->ed_time)) return NULL;
