              ...
              int R, float *lagpwr, double pwr, struct RadarParm *prm)
{
  ...
  /*f1=calc_err(w_guess-180./120.,raw,good_lags,goodcnt,R,lagpwr,pwr,prm);
  f2=calc_err(w_guess,raw,good_lags,goodcnt,R,lagpwr,pwr,prm);
  f3=calc_err(w_guess+180./120.,raw,good_lags,goodcnt,R,lagpwr,pwr,prm);
  fprintf(stderr, "bisect %f %f %f %f %f\n", w_guess, diff, f1, f2, f3);*/
  ...
  for (R=0;R<prm->nrang;R++)
  {
    ...
    if((pwr_flg) && (availcnt>=minlag))
    {
      ...
      if(sct_flg)
      {
        ...
        w_guess = bisect(w_guess,diff,raw,good_lags,goodcnt,R,lagpwr,pwr,prm);
